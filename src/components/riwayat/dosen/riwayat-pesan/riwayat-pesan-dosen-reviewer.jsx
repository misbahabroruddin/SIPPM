...
import { useQueryGetRiwayatPesanDosenReviewer } from "@/handlers/dosen/riwayat-pesan/reviewer/query-get-pesan-dosen-reviewer";
import { useCreatePesanDosenReviewer } from "@/handlers/dosen/riwayat-pesan/reviewer/add-pesan-dosen-reviewer";

export const RiwayatPesanDosenReviewer = ({
  riwayatId,
  status,
  catatan,
  index,
}) => {
  const { data } = useQueryGetRiwayatPesanDosenReviewer(riwayatId);
  const { data: session } = useSession();
  const { register, handleSubmit, reset } = useForm();
  const { onSubmitChat, isLoadingSubmit } = useCreatePesanDosenReviewer(
    riwayatId,
    reset,
  );
